...
def create_streaming_writer(
    ...
    if not staging_volume:
        raise ValueError(
            "staging_volume is REQUIRED when running outside of Databricks "
            "(or when forcing SQL mode). "
            "Because we cannot access local disk from the warehouse, "
            "we must stage files in a Volume."
        )
